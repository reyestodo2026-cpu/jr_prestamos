...
const App = {
    ...
    // ==========================================
    CODIGO_MAESTRO: 'JR*Maestro2026',

    mostrarRecuperacion() {
        document.getElementById('form-login').style.display = 'none';
        document.getElementById('panel-recuperacion').style.display = 'block';
        document.getElementById('recuperacion-error').style.display = 'none';
        document.getElementById('recuperacion-ok').style.display = 'none';
        document.getElementById('codigo-maestro').value = '';
        document.getElementById('codigo-maestro').focus();
    },
    ...
    toggleVerCodigo() {
        const input = document.getElementById('codigo-maestro');
        const icon  = document.getElementById('ico-codigo');
    ...
    async procesarRecuperacion() {
        const codigo = document.getElementById('codigo-maestro').value;
        const errorDiv = document.getElementById('recuperacion-error');
        const okDiv    = document.getElementById('recuperacion-ok');

        errorDiv.style.display = 'none';
        okDiv.style.display = 'none';

        if (codigo !== this.CODIGO_MAESTRO) {
            errorDiv.style.display = 'flex';
            return;
        }

        // Borrar credenciales guardadas → vuelven a funcionar las originales
        localStorage.removeItem('jhon_credenciales');
        if (FirebaseSync.saveCredenciales) FirebaseSync.saveCredenciales(null);

        okDiv.style.display = 'flex';
        document.getElementById('codigo-maestro').value = '';

        // Volver al login despues de 2 segundos
        setTimeout(() => {
            this.ocultarRecuperacion();
        }, 2500);
    },
